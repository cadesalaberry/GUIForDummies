...
public class Trie {
	...
	private TrieNode getPrefixNode(String word) {
		...
		return cur; // word is already a rooted path in the trie i.e. a path
					// starting at root
	...
	public boolean contains(String key) {
		...
		TrieNode prefixNode = getPrefixNode(key);
		return (prefixNode != null && prefixNode.getDepth() == key.length() && prefixNode
				.isEndOfKey());
	...
	public ArrayList<String> getAllPrefixMatches(String prefix) {
		...
		if (node.getDepth() == prefix.length()) // make sure whole prefix is
												// matched
